...
func TwoNumberSum2(array []int, target int) []int {
	...
	// iterating over an leetcode.array
	for i := 0; i < len(array); i++ {
		// find the mirror num, ex: target = 10 leetcode.array[i] = 3 mirrorNum = 10 - 3 = 7
		mirrorNum := target - array[i]
		if _, ok := m[mirrorNum]; ok {
			arr = append(arr, array[i], mirrorNum)
		} else {
			// add leetcode.array[i] to match pair if 10 - 7 == 3 and 3 would be inside the map
			m[array[i]] = nil
		}
	}
	return arr
